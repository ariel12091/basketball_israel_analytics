-- EUROLEAGUE SHADOW SCHEMA -- candidate B for migration 045.
--
...
--
-- Candidate B additionally collapses the two-stage agg -> pivoted pipeline into
-- one aggregation at the output key (player_id, team_id) using FILTER
-- expressions for the four offense/defense x ON/OFF combinations. `max(x)` over
-- a group that held exactly one row equals that row, so the values are
-- unchanged; an absent combination still yields NULL (an empty sum and an empty
-- max are both NULL). All numeric casts and NULLIF denominator guards are
-- preserved verbatim; four_factors_compute still divides only after summing.
--
...
BEGIN
  ...
  pivoted AS (
    SELECT
      c.player_id, c.team_id,
      (sum(c.total_points) FILTER (WHERE c.type_lineup = 'offense' AND c.is_on_key = 1))::numeric AS off_on_pts,
      (sum(c.total_poss) FILTER (WHERE c.type_lineup = 'offense' AND c.is_on_key = 1))::numeric AS off_on_poss,
      (sum(c.total_points) FILTER (WHERE c.type_lineup = 'offense' AND c.is_on_key = 0))::numeric AS off_off_pts,
      (sum(c.total_poss) FILTER (WHERE c.type_lineup = 'offense' AND c.is_on_key = 0))::numeric AS off_off_poss,
      (sum(c.total_points) FILTER (WHERE c.type_lineup = 'defense' AND c.is_on_key = 1))::numeric AS def_on_pts,
      (sum(c.total_poss) FILTER (WHERE c.type_lineup = 'defense' AND c.is_on_key = 1))::numeric AS def_on_poss,
      (sum(c.total_points) FILTER (WHERE c.type_lineup = 'defense' AND c.is_on_key = 0))::numeric AS def_off_pts,
      (sum(c.total_poss) FILTER (WHERE c.type_lineup = 'defense' AND c.is_on_key = 0))::numeric AS def_off_poss,
      (sum(c.onoff_minutes) FILTER (WHERE c.type_lineup = 'offense' AND c.is_on_key = 1))::numeric AS mins_on,
      (sum(c.fg2_made) FILTER (WHERE c.type_lineup = 'offense' AND c.is_on_key = 1))::bigint AS off_on_fg2_made,
      (sum(c.fg2_att) FILTER (WHERE c.type_lineup = 'offense' AND c.is_on_key = 1))::bigint AS off_on_fg2_att,
      (sum(c.fg3_made) FILTER (WHERE c.type_lineup = 'offense' AND c.is_on_key = 1))::bigint AS off_on_fg3_made,
      (sum(c.fg3_att) FILTER (WHERE c.type_lineup = 'offense' AND c.is_on_key = 1))::bigint AS off_on_fg3_att,
      (sum(c.fg2_made) FILTER (WHERE c.type_lineup = 'offense' AND c.is_on_key = 0))::bigint AS off_off_fg2_made,
      (sum(c.fg2_att) FILTER (WHERE c.type_lineup = 'offense' AND c.is_on_key = 0))::bigint AS off_off_fg2_att,
      (sum(c.fg3_made) FILTER (WHERE c.type_lineup = 'offense' AND c.is_on_key = 0))::bigint AS off_off_fg3_made,
      (sum(c.fg3_att) FILTER (WHERE c.type_lineup = 'offense' AND c.is_on_key = 0))::bigint AS off_off_fg3_att,
      (sum(c.fg2_made) FILTER (WHERE c.type_lineup = 'defense' AND c.is_on_key = 1))::bigint AS def_on_fg2_made,
      (sum(c.fg2_att) FILTER (WHERE c.type_lineup = 'defense' AND c.is_on_key = 1))::bigint AS def_on_fg2_att,
      (sum(c.fg3_made) FILTER (WHERE c.type_lineup = 'defense' AND c.is_on_key = 1))::bigint AS def_on_fg3_made,
      (sum(c.fg3_att) FILTER (WHERE c.type_lineup = 'defense' AND c.is_on_key = 1))::bigint AS def_on_fg3_att,
      (sum(c.fg2_made) FILTER (WHERE c.type_lineup = 'defense' AND c.is_on_key = 0))::bigint AS def_off_fg2_made,
      (sum(c.fg2_att) FILTER (WHERE c.type_lineup = 'defense' AND c.is_on_key = 0))::bigint AS def_off_fg2_att,
      (sum(c.fg3_made) FILTER (WHERE c.type_lineup = 'defense' AND c.is_on_key = 0))::bigint AS def_off_fg3_made,
      (sum(c.fg3_att) FILTER (WHERE c.type_lineup = 'defense' AND c.is_on_key = 0))::bigint AS def_off_fg3_att
    ...
    GROUP BY c.player_id, c.team_id
  ),
  rated AS (
    SELECT
      p.*,
...
BEGIN
  ...
  totals AS (
    SELECT
      c.player_id, c.team_id,
      (sum(c.total_points) FILTER (WHERE c.type_lineup = 'offense' AND c.is_on_key = 1))::numeric AS off_on_pts,
      (sum(c.total_poss) FILTER (WHERE c.type_lineup = 'offense' AND c.is_on_key = 1))::bigint AS off_on_poss,
      (sum(c.ts_poss_count) FILTER (WHERE c.type_lineup = 'offense' AND c.is_on_key = 1))::bigint AS off_on_ts_poss,
      (sum(c.oreb_count) FILTER (WHERE c.type_lineup = 'offense' AND c.is_on_key = 1))::bigint AS off_on_oreb_ct,
      (sum(c.oreb_opportunities) FILTER (WHERE c.type_lineup = 'offense' AND c.is_on_key = 1))::bigint AS off_on_oreb_opp,
      (sum(c.tov_count) FILTER (WHERE c.type_lineup = 'offense' AND c.is_on_key = 1))::bigint AS off_on_tov_ct,
      (sum(c.total_ft_attempts) FILTER (WHERE c.type_lineup = 'offense' AND c.is_on_key = 1))::bigint AS off_on_fta,
      (sum(c.total_fga) FILTER (WHERE c.type_lineup = 'offense' AND c.is_on_key = 1))::bigint AS off_on_fga,
      (sum(c.total_fgm) FILTER (WHERE c.type_lineup = 'offense' AND c.is_on_key = 1))::bigint AS off_on_fgm,
      (sum(c.total_fg3_made) FILTER (WHERE c.type_lineup = 'offense' AND c.is_on_key = 1))::bigint AS off_on_fg3m,
      (sum(c.total_points) FILTER (WHERE c.type_lineup = 'offense' AND c.is_on_key = 0))::numeric AS off_off_pts,
      (sum(c.total_poss) FILTER (WHERE c.type_lineup = 'offense' AND c.is_on_key = 0))::bigint AS off_off_poss,
      (sum(c.ts_poss_count) FILTER (WHERE c.type_lineup = 'offense' AND c.is_on_key = 0))::bigint AS off_off_ts_poss,
      (sum(c.oreb_count) FILTER (WHERE c.type_lineup = 'offense' AND c.is_on_key = 0))::bigint AS off_off_oreb_ct,
      (sum(c.oreb_opportunities) FILTER (WHERE c.type_lineup = 'offense' AND c.is_on_key = 0))::bigint AS off_off_oreb_opp,
      (sum(c.tov_count) FILTER (WHERE c.type_lineup = 'offense' AND c.is_on_key = 0))::bigint AS off_off_tov_ct,
      (sum(c.total_ft_attempts) FILTER (WHERE c.type_lineup = 'offense' AND c.is_on_key = 0))::bigint AS off_off_fta,
      (sum(c.total_fga) FILTER (WHERE c.type_lineup = 'offense' AND c.is_on_key = 0))::bigint AS off_off_fga,
      (sum(c.total_fgm) FILTER (WHERE c.type_lineup = 'offense' AND c.is_on_key = 0))::bigint AS off_off_fgm,
      (sum(c.total_fg3_made) FILTER (WHERE c.type_lineup = 'offense' AND c.is_on_key = 0))::bigint AS off_off_fg3m,
      (sum(c.total_points) FILTER (WHERE c.type_lineup = 'defense' AND c.is_on_key = 1))::numeric AS def_on_pts,
      (sum(c.total_poss) FILTER (WHERE c.type_lineup = 'defense' AND c.is_on_key = 1))::bigint AS def_on_poss,
      (sum(c.ts_poss_count) FILTER (WHERE c.type_lineup = 'defense' AND c.is_on_key = 1))::bigint AS def_on_ts_poss,
      (sum(c.oreb_count) FILTER (WHERE c.type_lineup = 'defense' AND c.is_on_key = 1))::bigint AS def_on_oreb_ct,
      (sum(c.oreb_opportunities) FILTER (WHERE c.type_lineup = 'defense' AND c.is_on_key = 1))::bigint AS def_on_oreb_opp,
      (sum(c.tov_count) FILTER (WHERE c.type_lineup = 'defense' AND c.is_on_key = 1))::bigint AS def_on_tov_ct,
      (sum(c.total_ft_attempts) FILTER (WHERE c.type_lineup = 'defense' AND c.is_on_key = 1))::bigint AS def_on_fta,
      (sum(c.total_fga) FILTER (WHERE c.type_lineup = 'defense' AND c.is_on_key = 1))::bigint AS def_on_fga,
      (sum(c.total_fgm) FILTER (WHERE c.type_lineup = 'defense' AND c.is_on_key = 1))::bigint AS def_on_fgm,
      (sum(c.total_fg3_made) FILTER (WHERE c.type_lineup = 'defense' AND c.is_on_key = 1))::bigint AS def_on_fg3m,
      (sum(c.total_points) FILTER (WHERE c.type_lineup = 'defense' AND c.is_on_key = 0))::numeric AS def_off_pts,
      (sum(c.total_poss) FILTER (WHERE c.type_lineup = 'defense' AND c.is_on_key = 0))::bigint AS def_off_poss,
      (sum(c.ts_poss_count) FILTER (WHERE c.type_lineup = 'defense' AND c.is_on_key = 0))::bigint AS def_off_ts_poss,
      (sum(c.oreb_count) FILTER (WHERE c.type_lineup = 'defense' AND c.is_on_key = 0))::bigint AS def_off_oreb_ct,
      (sum(c.oreb_opportunities) FILTER (WHERE c.type_lineup = 'defense' AND c.is_on_key = 0))::bigint AS def_off_oreb_opp,
      (sum(c.tov_count) FILTER (WHERE c.type_lineup = 'defense' AND c.is_on_key = 0))::bigint AS def_off_tov_ct,
      (sum(c.total_ft_attempts) FILTER (WHERE c.type_lineup = 'defense' AND c.is_on_key = 0))::bigint AS def_off_fta,
      (sum(c.total_fga) FILTER (WHERE c.type_lineup = 'defense' AND c.is_on_key = 0))::bigint AS def_off_fga,
      (sum(c.total_fgm) FILTER (WHERE c.type_lineup = 'defense' AND c.is_on_key = 0))::bigint AS def_off_fgm,
      (sum(c.total_fg3_made) FILTER (WHERE c.type_lineup = 'defense' AND c.is_on_key = 0))::bigint AS def_off_fg3m,
      (sum(c.steal_count) FILTER (WHERE c.type_lineup = 'defense' AND c.is_on_key = 1))::bigint AS def_on_steals,
      (sum(c.deflection_count) FILTER (WHERE c.type_lineup = 'defense' AND c.is_on_key = 1))::bigint AS def_on_deflections,
      (sum(c.steal_count) FILTER (WHERE c.type_lineup = 'defense' AND c.is_on_key = 0))::bigint AS def_off_steals,
      (sum(c.deflection_count) FILTER (WHERE c.type_lineup = 'defense' AND c.is_on_key = 0))::bigint AS def_off_deflections
    FROM euroleague.player_four_factors_by_game c
    JOIN games g ON g.game_id = c.game_id AND g.team_id = c.team_id
    WHERE (p_num_starters_off_min IS NULL OR c.own_starters >= p_num_starters_off_min)
      AND (p_num_starters_off_max IS NULL OR c.own_starters <= p_num_starters_off_max)
  ...
  pivoted AS (
    SELECT
      t.player_id, t.team_id,
      t.off_on_pts / NULLIF(2 * t.off_on_ts_poss, 0)::numeric AS off_on_ts,
      t.off_off_pts / NULLIF(2 * t.off_off_ts_poss, 0)::numeric AS off_off_ts,
      t.def_on_pts / NULLIF(2 * t.def_on_ts_poss, 0)::numeric AS def_on_ts,
      t.def_off_pts / NULLIF(2 * t.def_off_ts_poss, 0)::numeric AS def_off_ts,
      (t.off_on_fgm + 0.5 * t.off_on_fg3m) / NULLIF(t.off_on_fga, 0)::numeric AS off_on_efg,
      (t.off_off_fgm + 0.5 * t.off_off_fg3m) / NULLIF(t.off_off_fga, 0)::numeric AS off_off_efg,
      (t.def_on_fgm + 0.5 * t.def_on_fg3m) / NULLIF(t.def_on_fga, 0)::numeric AS def_on_efg,
      (t.def_off_fgm + 0.5 * t.def_off_fg3m) / NULLIF(t.def_off_fga, 0)::numeric AS def_off_efg,
      t.off_on_oreb_ct / NULLIF(t.off_on_oreb_opp, 0)::numeric AS off_on_oreb,
      t.off_off_oreb_ct / NULLIF(t.off_off_oreb_opp, 0)::numeric AS off_off_oreb,
      t.def_on_oreb_ct / NULLIF(t.def_on_oreb_opp, 0)::numeric AS def_on_oreb,
      t.def_off_oreb_ct / NULLIF(t.def_off_oreb_opp, 0)::numeric AS def_off_oreb,
      t.off_on_tov_ct / NULLIF(t.off_on_poss, 0)::numeric AS off_on_tov,
      t.off_off_tov_ct / NULLIF(t.off_off_poss, 0)::numeric AS off_off_tov,
      t.def_on_tov_ct / NULLIF(t.def_on_poss, 0)::numeric AS def_on_tov,
      t.def_off_tov_ct / NULLIF(t.def_off_poss, 0)::numeric AS def_off_tov,
      (t.def_on_steals + t.def_on_deflections) / NULLIF(t.def_on_poss, 0)::numeric AS def_on_disr,
      (t.def_off_steals + t.def_off_deflections) / NULLIF(t.def_off_poss, 0)::numeric AS def_off_disr,
      t.off_on_fta / NULLIF(t.off_on_fga, 0)::numeric AS off_on_ftr,
      t.off_off_fta / NULLIF(t.off_off_fga, 0)::numeric AS off_off_ftr,
      t.def_on_fta / NULLIF(t.def_on_fga, 0)::numeric AS def_on_ftr,
      t.def_off_fta / NULLIF(t.def_off_fga, 0)::numeric AS def_off_ftr,
      t.off_on_poss,
      t.off_off_poss,
      t.def_on_poss,
      t.def_off_poss
