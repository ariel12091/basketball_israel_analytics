-- EuroLeague shadow schema -- migration 044: custom Player Stats
-- function-only execution-shape optimization. No table, column, index, or
-- publication change.
...
lineup_identities AS (
  SELECT DISTINCT ol.game_id,ol.team_id,ol.own_lineup,l.player_ids
  FROM observed_lineups ol
  JOIN euroleague.lineup_totals_by_game l
    ON l.game_id=ol.game_id
   AND l.team_id=ol.team_id
   AND l.own_lineup=ol.own_lineup
   AND l.type_lineup='offense'
),
lineup_map AS MATERIALIZED (
  SELECT li.game_id,li.team_id,li.own_lineup,u.player_id
  FROM lineup_identities li
  CROSS JOIN LATERAL unnest(li.player_ids) u(player_id)
),
