...
-- WHAT CHANGES
--
-- 1. The analytical aggregation in both functions reads the base fact
--    euroleague.player_four_factors_by_game instead of the
--    euroleague.player_game_context view. That is the ONLY edit to either body:
--    the relation alias `c` is retained, so no other token differs from the
--    definitions in sql/004_app_read_layer.sql.
--
--    The view joined euroleague.schedule and the two-perspective
--    euroleague.final_schedule to every fact row, then discarded the unwanted
--    perspective -- a primary-key probe plus a CROSS JOIN LATERAL VALUES
--    expansion per row, none of whose columns the aggregation consumes. The
--    `games` CTE already resolves every schedule filter, so joining the fact to
--    `games` on (game_id, team_id) is the same restriction at a fraction of the
...
--      44 (ff) accumulators per group no longer fit in work_mem and introduced
--      an on-disk sort where there was none; 2-3x slower than this migration.
--   C  B plus 16MB work_mem -- removed the spill but still ~2.5x slower than
--      this migration, so the wide aggregate costs more CPU than two narrow
--      passes.
--
-- euroleague.player_game_context is NOT dropped here: the season aggregates in
-- migration 002 still read it.
