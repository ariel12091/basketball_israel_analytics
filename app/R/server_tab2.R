...
server_tab2 <- function(input, output, session, shared) {
  ...
  ld_params <- reactive({
    ...
    # player_csv carries every on-player; player_required_csv marks the subset
    # that must ALL be present, leaving the rest as one "at least one of"
    # clause. Both boxes empty, or only the required one filled, reproduces the
    # historical all-of filter exactly.
    player_req_ids <- if (!is.na(team_id)) as.integer(ld_lineup_filter$players_on()) else integer(0)
  ...
  output$ld_table <- DT::renderDataTable({
    ...
    if (identical(mode, "Four Factors")) {
      lineup_ff_datatable(df, ld_stat_filter_state$filters(),
                          LD_LINEUP_TABLE_SPEC, raw = ld_data())
    } else {
      lineup_summary_datatable(df, ld_stat_filter_state$filters(),
                               LD_LINEUP_TABLE_SPEC)
    }
  })
  ...
  observeEvent(input$ld_clear_players_on_any, {
    updateSelectizeInput(session, "ld_lineup_filter-players_on_any", selected = character(0))
  }, ignoreInit = TRUE)
