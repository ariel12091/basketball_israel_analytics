...
# URL bookmarking marks restored requests with a standalone `_inputs_` query
# component. Those requests must rebuild the UI inside their restore context so
# Shiny's native restoreInput() can select static controls such as main_tabs.
is_bookmark_request <- function(request) {
  query <- tryCatch(request$QUERY_STRING %||% "", error = function(e) "")
  if (!nzchar(query)) return(FALSE)
  query <- sub("?", "", query, fixed = TRUE)
  "_inputs_" %in% strsplit(query, "&", fixed = TRUE)[[1]]
}
...
# ---- Player Stats (traditional) shared helpers ----
# Moved verbatim from server_tab5_traditional.R (the three closures were
# de-indented only) so the On/Off tab's Player Stats filter chips reuse the
# Player Stats tab's own normalisation and per-game formulas instead of a
# second copy. None of them reads input or session state.
...
build_stint_ribbon_svg <- function(lanes, margin, meta, id_prefix = "ribbon",
  ...
  tags$svg(
    ...
    `data-period-bounds` = paste(bounds, collapse = ","),
