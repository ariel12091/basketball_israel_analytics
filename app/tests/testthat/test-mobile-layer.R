...
test_that("quarter cards draw at one uniform scale and drop idle rows", {
  ...
  # Players who sat out a period get no blank row, and every bar -- its
  # number, tap detail and lineups -- describes that period only.
  expect_true(grepl("collapseRows(svg, source);", js, fixed = TRUE))
  expect_true(grepl("clipBarsToPeriod(svg, index,", js, fixed = TRUE))
  expect_true(grepl("lane.dataset.segments = detail.segments;", js, fixed = TRUE))
