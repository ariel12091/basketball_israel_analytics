test_that("tab7 exact storyline rating presets use the persisted cache", {
  ...
  shiny::testServer(function(input, output, session) {
    ...
    session$setInputs(
      ...
      cmp_mode = "Teams",
      cmp_metric = "net_rtg",
      cmp_a_starters_mode = "gte",
      cmp_a_starters_val = "3",
      cmp_b_starters_mode = "lte",
      cmp_b_starters_val = "2"
    )
    session$elapse(300)
    session$flushReact()

    expect_equal(output$cmp_summary_a_label, "Net Rtg")
    expect_gt(mock_db_query_count("team_ratings_preset_cache"), 0L)
    expect_equal(mock_db_query_count("team_ratings_dynamic"), 0L)
  })
})
