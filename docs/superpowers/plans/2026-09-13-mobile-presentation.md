...
1. **Row tap cannot be the expand affordance.** Both Compare (`server_tab7_compare.R:3324`, `table.on('click', 'tbody tr', ...)` → `cmp_team_players_player_click`) and Tab 2 (`input$ld_lineup_click`, `server_tab2.R:478`) already bind whole-row clicks. The expand control is therefore a **dedicated caret button** in the first priority cell — which is what the approved mockup showed (`+ Avdija`).
