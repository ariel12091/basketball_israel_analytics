-- Incremental refresh for sub_lineups_stats scoped to affected game_ids.
-- Use this in ETL single-game/backfill runs to avoid full-table recompute.

CREATE OR REPLACE FUNCTION basketball_test.refresh_sub_lineups_stats_for_games(p_game_ids int4[])
 RETURNS bigint
 LANGUAGE sql
...
target_lineups AS (
  SELECT DISTINCT
    ...
    l.team_id,
    l.game_year
  FROM basketball_test.lineups_lookup l
  WHERE EXISTS (
    SELECT 1 FROM basketball_test.lineups_lookup touched
    WHERE touched.game_id = ANY(p_game_ids)
      AND touched.team_id = l.team_id AND touched.game_year = l.game_year
  )
...
fr_latest AS (
  ...
  GROUP BY fr.team_id, fr.player_id, fr.game_year
),
...
sub_dedup_234 AS (
  ...
  JOIN target_lineups tl
    ON tl.lineup_hash = s.lineup_hash
   AND tl.team_id = s.team_id
   AND tl.game_year = s.game_year
...
full_lineups AS (
  ...
  JOIN target_lineups tl
    ON tl.lineup_hash = l.lineup_hash
   AND tl.team_id = l.team_id
   AND tl.game_year = l.game_year
...
names_by_sub AS (
  SELECT
    ...
    d.sub_lineup_hash,
    d.game_year,
    ARRAY_AGG(COALESCE(fr.name, '#' || u.player_id::text) ORDER BY u.ord) AS player_names,
    STRING_AGG(COALESCE(fr.name, '#' || u.player_id::text), ', ' ORDER BY u.ord) AS player_names_str
  FROM sub_dedup d
  ...
  GROUP BY d.team_id, d.sub_lineup_hash, d.game_year
),
...
needed_lineups AS (
  SELECT DISTINCT lineup_hash, game_year
  FROM sub_map
),
...
segment_times AS (
  ...
  GROUP BY f.lineup_hash, f.game_id, sc.game_year, f.segment_id
),
...
segment_stats AS (
  ...
  JOIN needed_lineups nl
    ON nl.lineup_hash = f.lineup_hash
   AND nl.game_year = sc.game_year
  GROUP BY f.lineup_hash, f.game_id, sc.game_year, f.type_lineup, f.segment_id
),
...
lineup_totals AS (
  ...
  GROUP BY ss.lineup_hash, ss.game_year, ss.type_lineup
),
...
per_type AS (
  ...
  GROUP BY sm.team_id, sm.sub_lineup_hash, sm.game_year, lt.type_lineup
),
